import { useRef, useEffect } from 'react';
import { gsap } from 'gsap';
import { Observer } from 'gsap/Observer';
import './InfiniteScroll.css';

gsap.registerPlugin(Observer);

export default function InfiniteScroll({
  width = '30rem',
  maxHeight = '100%',
  negativeMargin = '-0.5em',
  items = [],
  itemMinHeight = 150,
  isTilted = false,
  tiltDirection = 'left',
  autoplay = false,
  autoplaySpeed = 0.5,
  autoplayDirection = 'down',
  pauseOnHover = false
}) {
  const wrapperRef = useRef(null);
  const containerRef = useRef(null);

  const getTiltTransform = () => {
    if (!isTilted) return 'none';
    return tiltDirection === 'left'
      ? 'rotateX(20deg) rotateZ(-20deg) skewX(20deg)'
      : 'rotateX(20deg) rotateZ(20deg) skewX(-20deg)';
  };

  useEffect(() => {
    const container = containerRef.current;
    if (!container) return;
    if (items.length === 0) return;

    const divItems = gsap.utils.toArray(container.children);
    if (!divItems.length) return;

    const firstItem = divItems[0];
    const itemStyle = getComputedStyle(firstItem);
    const itemHeight = firstItem.offsetHeight;
    const itemMarginTop = parseFloat(itemStyle.marginTop) || 0;
    const totalItemHeight = itemHeight + itemMarginTop;
    const totalHeight = itemHeight * items.length + itemMarginTop * (items.length - 1);

    const wrapFn = gsap.utils.wrap(-totalHeight, totalHeight);

    divItems.forEach((child, i) => {
      const y = i * totalItemHeight;
      gsap.set(child, { y });
    });

    let currentRafId: number | null = null;

    const speed = autoplaySpeed; // autoplaySpeed is now always positive

    const tick = () => {
      divItems.forEach(child => {
        const currentY = gsap.getProperty(child, "y");
        let newY;
        if (autoplayDirection === 'down') {
          newY = currentY + speed;
        } else {
          newY = currentY - speed;
        }
        gsap.set(child, {
          y: newY,
          modifiers: {
            y: gsap.utils.unitize(wrapFn)
          }
        });
      });
      currentRafId = requestAnimationFrame(tick);
    };

    const stopTicker = () => {
      if (currentRafId) {
        cancelAnimationFrame(currentRafId);
        currentRafId = null;
      }
    };

    const startTicker = () => {
      if (autoplay && !currentRafId) {
        currentRafId = requestAnimationFrame(tick);
      }
    };

    if (autoplay) {
      startTicker();

      if (pauseOnHover) {
        container.addEventListener('mouseenter', stopTicker);
        container.addEventListener('mouseleave', startTicker);
      }
    }

    const observer = Observer.create({
      target: container,
      type: 'wheel,touch,pointer',
      preventDefault: true,
      onPress: ({ target }) => {
        target.style.cursor = 'grabbing';
        stopTicker(); // Stop autoplay on manual interaction
      },
      onRelease: ({ target }) => {
        target.style.cursor = 'grab';
        startTicker(); // Resume autoplay after manual interaction
      },
      onChange: ({ deltaY, isDragging, event }) => {
        const d = event.type === 'wheel' ? -deltaY : deltaY;
        const distance = isDragging ? d * 1 : d * 2;

        divItems.forEach(child => {
          gsap.to(child, {
            duration: 1.5,
            ease: 'expo.out',
            y: `+=${distance}`,
            modifiers: {
              y: gsap.utils.unitize(wrapFn)
            }
          });
        });
      }
    });

    return () => {
      observer.kill();
      stopTicker();
      if (pauseOnHover) {
        container.removeEventListener('mouseenter', stopTicker);
        container.removeEventListener('mouseleave', startTicker);
      }
    };
  }, [items, autoplay, autoplaySpeed, autoplayDirection, pauseOnHover, isTilted, tiltDirection, negativeMargin]);

  return (
    <>
      <style>
        {`
        .infinite-scroll-wrapper {
          max-height: ${maxHeight};
        }

        .infinite-scroll-container {
          width: ${width};
        }

        .infinite-scroll-item {
          height: ${itemMinHeight}px;
          margin-top: ${negativeMargin};
        }
        `}
      </style>

      <div className="infinite-scroll-wrapper" ref={wrapperRef}>
        <div
          className="infinite-scroll-container"
          ref={containerRef}
          style={{
            transform: getTiltTransform()
          }}
        >
          {items.map((item, i) => (
            <div className="infinite-scroll-item" key={i}>
              {item.content}
            </div>
          ))}
        </div>
      </div>
    </>
  );
}